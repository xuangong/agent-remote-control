...
A retained iframe renews its registration and preview cookie, including while minimized or attached to another session on the same Host. Renewals are deduplicated by registration within each provider and run after one third of the remaining lease, at most five minutes apart. Each request has a 20-second deadline; transient failures retry after five seconds. Retained previews can renew by ID even when omitted from the active list. An HTTP 409 renewal rejection confirms that the registration is unavailable or unregistered and stops further attempts, including wake-triggered retries. Returning to a visible page, restoring a page from the browser cache, and coming online trigger an immediate attempt. Closing the last retained browser for a registration, changing Hosts, leaving the page, or receiving manual unregister stops renewal. Without renewals the Controller expires the lease normally.
