...
async function setup(localProviders: 'ready' | 'empty' | 'error' = 'ready') {
  ...
  vi.stubGlobal('fetch', async (input: URL | string, init?: RequestInit) => {
    ...
    if (url.pathname === '/v1/providers') return Response.json(localProviders === 'error'
      ...
      : { protocolVersion: '1.1.0', type: 'provider_list', payload: { providers: localProviders === 'empty' ? [] : [{ providerId: 'recorded', displayName: 'Recorded semantic Provider' }] } }, { status: localProviders === 'error' ? 503 : 200 });
