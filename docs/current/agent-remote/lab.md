...
A tab attached to the input dock toggles message input visibility. Collapsing leaves the tab at the bottom and gives the recovered space to the Timeline; the mounted composer retains its draft. Expanding restores the same input without automatically opening the keyboard. The Timeline preserves its reading anchor or follows the latest content as the dock changes height.
