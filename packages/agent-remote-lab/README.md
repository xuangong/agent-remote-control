...
`@orchardworks/agent-remote-lab` is a local chat and protocol debugging console. Run its DSH plugin inside DSH Web to use the same native session from both interfaces, then open the Lab to inspect the public protocol and client Replica. The console uses shared React components from `@orchardworks/agent-remote-web`; it is not a Borgee product service or a complete Provider control plane.
...
"$LAB_PNPM" --filter @orchardworks/agent-remote-lab... run build
"$LAB_PNPM" --filter @orchardworks/agent-remote-lab run build:dsh-plugin
export LAB_DSH_PLUGIN="$LAB_ROOT/packages/agent-remote-lab/dist/dsh-plugin/orchardworks-agent-remote-lab-dsh-0.1.0.tgz"
test -f "$LAB_DSH_PLUGIN"
```

The package is `@orchardworks/agent-remote-lab-dsh`. Its declared DSH bundle inserts the Agent Remote plugin into the Web profile, injects `sessionController`, `agentPresets`, `userQuestions`, and `approval`, and selects `runtimeMode: shared-web`. Installing it with `dsh plugin` activates that bundle; no hand-written patch or global configuration change is needed.

This debugging package bundles the standalone adapter and Relay while using the DSH installation's native services. It carries its own compatibility manifest and registers the real DSH Provider only. It does not require a Codex executable. The independent outbound Host bundle uses `@orchardworks/dsh-host` and connects to the workbench broker.
...
```bash
"$LAB_PNPM" --filter @orchardworks/agent-remote-lab run prepare:dsh-release "$LAB_RUNTIME/dsh-release"
...
```bash
cd "$LAB_ROOT"
env -u DSH_REPO -u BORGEE_CODEX_TEST_EXECUTABLE /usr/bin/perl -e 'alarm 240; exec @ARGV' "$LAB_PNPM" --filter @orchardworks/agent-remote-lab exec playwright test e2e/recorded.spec.ts --project=chromium-desktop --project=chromium-mobile --timeout=120000
```
...
```bash
pnpm --filter @orchardworks/agent-remote-lab run test:conformance
```
...
test "$(git -C "$DSH_REPO" rev-parse HEAD)" = 'a66e4702047846cdaa10c66c9d3df3951f5ea70d'
"$BORGEE_CODEX_TEST_EXECUTABLE" --version
/usr/bin/perl -e 'alarm 600; exec @ARGV' "$LAB_PNPM" --filter @orchardworks/agent-remote-lab exec playwright test e2e/live-dsh.spec.ts --project=chromium-desktop --project=chromium-mobile --timeout=360000
```
...
```bash
cd "$LAB_ROOT"
/usr/bin/perl -e 'alarm 180; exec @ARGV' "$LAB_PNPM" --filter @orchardworks/agent-remote-lab exec vitest run src/server/compatibility.test.ts src/server/live-launcher.test.ts --testTimeout=10000
/usr/bin/perl -e 'alarm 180; exec @ARGV' "$LAB_PNPM" --filter @orchardworks/agent-remote-lab run typecheck
/usr/bin/perl -e 'alarm 180; exec @ARGV' "$LAB_PNPM" --filter @orchardworks/agent-remote-lab run build
```
