...
describe('DSH Remote Host plugin', () => {
  ...
  it('projects only exact top-level native Agents and retains them after Remote Host transport closure', async () => {
    ...
    expect((await remote.request('/v1/sessions/binding-one/snapshot?protocolVersion=1.3.0', 'binding-one')).status).toBe(200);
    ...
    await vi.waitFor(async () => expect((await remote.request('/v1/sessions/binding-one/snapshot?protocolVersion=1.3.0', 'binding-one')).status).toBe(403));
  ...
  it('resumes a persisted native session when Remote Host attaches after a cold start', async () => {
    ...
    expect((await remote.request('/v1/sessions/binding-cold/snapshot?protocolVersion=1.3.0', 'binding-cold')).status).toBe(200);
  ...
  it('projects live native events from an attached Remote Host session', async () => {
    ...
    remote.sendStream('native-events', { protocolVersion: '1.3.0', type: 'negotiate' });
    await vi.waitFor(() => expect(remote.streamFrames('native-events')).toContainEqual(expect.objectContaining({
      type: 'agent_snapshot',
    })), { timeout: 3000 });
    remote.sendStream('native-events', { protocolVersion: '1.3.0', type: 'timeline_subscription', payload: {
  ...
  it('reads exact native metadata without attaching or restoring a session', async () => {
    ...
    expect(native.resolveCalls).toEqual([]);
    expect(native.createCalls).toEqual([]);
    expect((await remote.request('/v1/sessions/unbound/snapshot?protocolVersion=1.3.0', 'unbound')).status).toBe(403);
