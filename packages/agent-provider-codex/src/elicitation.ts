...
import type { AgentFormField, AgentInteractionRequest, AgentInteractionResponse } from '@borgee/agent-provider-sdk';
...
export function mapCodexElicitation(params: Record<string, unknown>, requestId: string): AgentInteractionRequest {
  ...
  const title = readString(params.serverName);
...
export function mapCodexElicitationResponse(response: AgentInteractionResponse): unknown {
  if (response.kind === 'form') return { action: response.action === 'submit' ? 'accept' : response.action, content: response.action === 'submit' ? response.values : null, _meta: null };
  if (response.kind === 'external_action') return { action: response.action === 'completed' ? 'accept' : response.action, content: null, _meta: null };
