...
type RecordKind = 'sessionMigration' | 'sessionStar' | 'preview' | 'securityEvent' | 'session' | 'tenant' | 'deviceKey' | 'host' | 'binding' | 'creation' | 'share' | 'reservation' | 'loginChallenge' | 'consumedProof';
type StoredRecord = { kind: RecordKind; owner: string; key: string; version: number; value: string };
const recordTable = (kind: RecordKind) => kind === 'sessionMigration' ? 'relay_session_migrations' : kind === 'sessionStar' ? 'relay_session_stars' : kind === 'preview' ? 'relay_previews' : kind === 'securityEvent' ? 'relay_security_events' : 'relay_records';
...
function decode(rows: StoredRecord[], auth: GatewayAuthOptions): HostedRelayState {
  ...
  for (const row of rows) {
    ...
    if (row.owner && !broker && row.kind !== 'sessionStar' && row.kind !== 'sessionMigration') throw new Error('Relay record owner is missing.');
    switch (row.kind) {
      case 'tenant': break;
