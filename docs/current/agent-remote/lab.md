...
- Composer drafts and pending commands belong to an Agent; successful send or steer confirmation clears only the submitted draft, rejection preserves it, and cancel leaves it intact. A completion for another Agent cannot redirect focus or replace the current draft (`packages/agent-remote-web/src/react/AgentComposer.tsx:21`).
