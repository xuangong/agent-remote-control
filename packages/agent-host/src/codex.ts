...
export async function createCodexHostRegistration(options: CodexHostRegistrationOptions = {}): Promise<AgentHostProviderRegistration> {
  ...
  return { preservesWorkOnDisconnect: options.connectionMode === 'shared',
    nativePermissionControl: options.connectionMode === 'shared' && !options.restrictedNative,
    adapter: provider, directory: createCodexSessionDirectory(provider, options.workspaces ?? [], references) };
}
