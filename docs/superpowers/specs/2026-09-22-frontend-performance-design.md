...
- Text drafts use a small keyed Lab store; the public composer still supports controlled/uncontrolled consumers. Scope generations reject late writes after sign-out, including Ask drafts.
- The scroll index invalidates through DOM entry mutations instead of requiring ref registration in every renderer consumer. It checks the current entry/neighbors and uses binary search for large jumps. Scroll events retain synchronous capture to preserve exact reading positions at navigation/prepend boundaries; the browser's native scroll cadence is used rather than adding a delayed animation-frame capture. Before-paint resize and content observers preserve same-height text replacements.
- Image bytes use a separate object store in database version 2. New bytes are stored as ArrayBuffer values, memoized per Blob, to avoid a reproduced WebKit Blob-preparation transaction failure. Metadata remains readable in the existing version-1 document shape; the reader accepts both embedded bytes and byte references. Transactions use `getKey` for byte existence, so caption edits neither read nor rewrite entire blobs. Removed tags can release durable bytes while the live editor retains their blobs for undo; undo persists the bytes again. This follows the existing recovery model, which persists current content rather than undo history.
- Clipboard cache misses asynchronously restore durable image bytes; the editor maps the captured paste selection through intervening edits and drops completion after session remount. Missing bytes remain an explicit unavailable-image error.
- Replica ownership is reconciled after React commits and eviction runs only there. Obtain/attach never evicts an entry being bound during render. Mounted panes and all unsettled outboxes are protected; inactive clean image drafts can be rehydrated.
- The authentication entry imports headless resume/storage helpers to avoid pulling the renderer through a barrel export. The existing digit font uses an external same-origin asset, preventing Vite's default data-URL inlining from violating the site's CSP.
- The first five-run comparison exceeded the 60% improvement goal (83.3% at 1,000 entries). The 1,000/100 total-task ratio was 1.60, slightly above the aspirational 1.50 target, while typing-induced timeline geometry reads were zero. The remaining DOM/browser cost is reported rather than hidden by truncation or virtualization.
