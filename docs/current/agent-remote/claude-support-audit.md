...
Audit refresh: **2026-09-25**, `feat/claude-normalized`, based on main `139a891`. Agent SDK **0.3.247**, Claude Code **2.1.247**. This is a source and native-test audit, not a release claim. See the [complete normalized event inventory](claude-normalized-events.md) for current event coverage, stdio ownership and recovery boundaries.
...
| S3, S5 | No attachment to another Claude terminal process. Managed CLI/Host takeover explicitly stops the old writer before resuming the same native ID. Model/effort/planning can be selected at creation; live model settings now use public Query discovery and setters. | [session.ts](../../../packages/agent-provider-claude/src/session.ts), [Host registration](../../../packages/agent-host/src/claude.ts). Terminal attachment remains unavailable; creation-only effort retains W2. |
...
| X4, X5, X6 | No MCP form, standalone granular permission_approval or external-action mapping. Suggested session tool rules do not imply general granular permission parity. Real native elicitation drops constraints and sensitive markers before the public callback. | [Native schema-loss probes](../../../packages/agent-provider-claude/src/native-elicitation.local.test.ts). Keep unavailable; W9 does not confer equivalence. |
...
| A4, A5 | SDK saved child history is reconciled by UUID/content-block identity. Fresh saved attachment is canonical; an already observed binding reconciles late native transcript prefixes through timeline replacement. Foreground observation ends at parent result; background tasks follow native terminal events; root loss freezes views. | [child-session.ts](../../../packages/agent-provider-claude/src/child-session.ts), [children tests](../../../packages/agent-provider-claude/src/children.test.ts). Partial; neither freeze nor saved-history status proves successful native work. W4. |
...
- **Protocol and Relay:** no new public rail is needed for implemented skills or child views. They reuse command list/result, runtime child descriptors and ordinary Agent sessions. Child transcript correction reuses the existing timeline replacement and epoch flow.
...
- **Debugger:** an already bound Claude root/child can be inspected using the public Agent ID. ARDB offers settings, send/cancel and typed interaction commands, plus live/replay Session View and JSONL recording. General native commands and Host child attachment still use the product controls.
...
| Reasoning settings and queue | G2/G4: no verified public live effort readback; native queued input can outlive a turn and public interrupt. | Creation effort only; no Query replacement workaround, hidden queue or hot-update claim. |
...
| Child controls and recovery | G11/G12: native task stop is a limited candidate; full child control and history rebase are distinct problems. | Children remain read-only; approvals belong to parent; late transcript prefixes use timeline replacement. |
| Debugger | ARDB now supplies live/replay, settings and interaction CLI controls. | Native skill menus and child navigation remain capability-gated product controls. |
...
## Managed stdio acceptance (2026-09-25)

The Claude directory shares the managed stdio lifecycle with Copilot. It resolves cwd by native ID even if the catalog omits a session, refuses unknown cwd, retains leases on unconfirmed shutdown and snapshots current persistence settings before transfer. The CLI wrapper participates for explicit native IDs. Direct unmanaged Claude processes cannot be detected through a public native lock probe.

Native tests use a real Claude Code 2.1.247 executable, isolated profiles/workspaces and loopback model responses. Coverage includes SDK → CLI → SDK while running, native Write/Edit diffs, session grants for a repeated ordinary command, and cold history limits. The ARDB mobile browser test exercises the same Session View with a real Query, approval through reconnect, actual diff, reload and width checks. Neither these tests nor mock Windows tests certify real Windows native handoff or online model-service behavior.
