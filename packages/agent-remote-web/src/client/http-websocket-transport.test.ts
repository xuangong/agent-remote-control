...
describe('HttpWebSocketTransport', () => {
  ...
  it('decodes public Snapshot and Timeline values from the HTTP endpoints', async () => {
    ...
    expect(calls).toEqual([
      'http://relay.test/base/v1/sessions/agent%20one/snapshot?protocolVersion=1.4.0',
      'http://relay.test/base/v1/sessions/agent%20one/timeline?protocolVersion=1.4.0&requestId=history-one&direction=after&limit=50&epoch=epoch-one&seq=1',
    ]);
  });
