...
export function reply(res: ServerResponse, body: ModelRequest, content: string | {name: string; arguments: object}) {
 ...
 res.end([chunk(tool ? {role: 'assistant', tool_calls: [{index: 0, id: `fixture-tool-${crypto.randomUUID()}`, type: 'function', function: {name: content.name, arguments: JSON.stringify(content.arguments)}}]} : {role: 'assistant', content}), chunk({}, tool ? 'tool_calls' : 'stop')].map(c => `data: ${JSON.stringify(c)}\n\n`).join('') + 'data: [DONE]\n\n');
}
...
export function observe(session: AgentSession) {
 ...
 const done = (async () => { for await (const item of session.observe()) items.push(item); })();
 return {items, done, events: () => items.flatMap(i => i.type === 'observation' ? [i.event] : [])};
}
