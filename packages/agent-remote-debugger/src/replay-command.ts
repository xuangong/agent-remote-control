...
export async function runReplayCommand(invocation: ParsedInvocation, io: DebuggerIo, signal: AbortSignal) {
  ...
  const recording = await (async () => {
    ...
    try {
      ...
      if (!stat.isFile() || stat.size > 64 * 1024 * 1024) fail('Select a regular JSONL file no larger than 64 MiB.');
      return parseRecording(await file.readFile('utf8'));
    } finally { await file.close(); }
  })().catch(error => fail(`Cannot replay ${basename(path)}: ${error instanceof Error ? error.message : String(error)}`));
  let stop = () => {};
  ...
  try {
    ...
    server = await createReplayServer({ recording, name: basename(path), port: Number(port) });
