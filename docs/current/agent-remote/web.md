...
- Failed tool calls keep their Failed status in the header. Preview mode shows a bounded error excerpt; Simple mode and explicitly collapsed calls show only the header. Error text already contained in a text output block is shown only in the output, in both preview and expanded views; distinct diagnostics remain visible. The existing disclosure reveals the complete, scrollable error alongside tool details and output, and later error updates preserve the reader's collapse choice (`packages/agent-remote-web/src/react/items/ToolCallItem.tsx`).
...
- While ready, acknowledged input missing its echo triggers a shared background history scan after 10 seconds, then 30 seconds, then once per minute. Each scan reads from the oldest pending send cursor, with at most eight pages and a 15-second read deadline; unfinished scans continue at the next check. Reads preserve newer live projections and never regress the live cursor. Missing echoes, native compaction, and read failures do not trigger resends or delivery failures. Echo arrival or connection teardown stops the checks. This repairs echoes present in the Controller Timeline; it does not force the native runtime to emit a missing event (`packages/agent-remote-web/src/client/message-echo-sync.ts`).
