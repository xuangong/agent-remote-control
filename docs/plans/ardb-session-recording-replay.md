...
4. Reuse LabWorkbench with explicitly read-only input and no operation callbacks. Add a compact playback toolbar with play/pause, speed, seek, step, restart and server-side JSONL file browsing. Do not reproduce browser connection toasts as new failures.
