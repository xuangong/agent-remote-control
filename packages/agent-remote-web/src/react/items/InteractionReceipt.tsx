...
export function InteractionReceipt({ request, response }: { request: AgentInteractionRequest; response: AgentInteractionResponse }) {
  if (request.kind === 'form' && response.kind === 'form') {
    return <article className="agent-item agent-interaction-completed">
      <header><span className="agent-item-kicker">FORM</span><span className="agent-state-label">{response.action === 'submit' ? 'Submitted' : response.action === 'cancel' ? 'Canceled' : 'Declined'}</span></header>
      <h3>{request.title}</h3>
  ...
  if (request.kind === 'permission_approval' && response.kind === 'permission_approval') {
    return <article className="agent-item agent-interaction-completed"><header><span className="agent-item-kicker">PERMISSIONS</span><span className="agent-state-label">{response.decision === 'allow' ? `Allowed · ${response.scope}` : 'Denied'}</span></header><p>{request.summary}</p><ul className="agent-permission-list">{request.permissions.map((permission, index) => <li key={index}><strong>{permission.access} · {permission.resource}</strong><code>{permission.target}</code></li>)}</ul></article>;
  }
  if (request.kind === 'external_action' && response.kind === 'external_action') {
    return <article className="agent-item agent-interaction-completed"><header><span className="agent-item-kicker">EXTERNAL ACTION</span><span className="agent-state-label">{response.action === 'completed' ? 'Completed' : response.action === 'cancel' ? 'Canceled' : 'Declined'}</span></header><p>{request.title}</p></article>;
  }
