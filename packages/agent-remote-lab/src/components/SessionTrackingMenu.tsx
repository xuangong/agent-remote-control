...
export function SessionTrackingMenu({ tracking, busy, inert, onOpen }: { tracking: SessionTracking; busy: boolean; inert: boolean; onOpen(session: SessionStar): void }) {
  const changes = tracking.sessions.filter(session => tracking.observations[sessionKey(session)]?.changed).length;
  const attention = tracking.sessions.some(session => ['waiting', 'failed'].includes(tracking.observations[sessionKey(session)]?.activity ?? ''));
  return <div className="lab-tracking-floating" data-attention={attention} {...(inert ? { inert: '' } : {})}>
    <SessionPopover label="Tracked sessions" trigger={<><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="M3 12h4l3-8 4 16 3-8h4" /></svg><span className="lab-tracking-count">{tracking.sessions.length}</span>{changes ? <span className="lab-tracking-badge" aria-label={`${changes} session status changes`}>{changes}</span> : attention ? <span className="lab-tracking-attention" aria-label="A tracked session needs attention" /> : null}</>} onOpen={tracking.acknowledge}>
      {close => <>
        <p className="lab-control-note">Tracking on this client</p>
        {!tracking.sessions.length ? <p className="lab-control-note">Choose Track in Favorites to watch a session here.</p> : null}
        <ul className="lab-favorite-list">{tracking.sessions.map(session => {
