...
export function ExternalActionCard({ request, onResponse, pending, failure, readOnly = false }: Props) {
  ...
  return <section className="agent-interaction agent-external-action" aria-busy={pending}>
    ...
    <p>{request.message}</p>
    {url && readOnly ? <p>Recorded link: {url}</p> : url ? <a className="agent-external-link" href={url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer">Open {new URL(url).host}<span aria-hidden="true"> ↗</span></a> : <p role="alert">The requested link cannot be opened.</p>}
    {url && !readOnly ? <small>Return here after completing the action.</small> : null}
