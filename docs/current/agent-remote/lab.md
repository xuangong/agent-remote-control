...
On compact screens, Sessions & Hosts keeps the account row, section navigation, and New session action outside a single scrolling body. Sessions shows the Host selector, collapsible environment filter/details, and session discovery first; previews follow the list and Controller update notices remain available above it. VS Code tunnel management is under Settings. Favorites and creation remain separate panels. Track is visually hidden while a supporting dialog is open without stopping its observations. Disclosure controls participate in the drawer focus loop; closed disclosure contents do not.
