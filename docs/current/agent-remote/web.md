...
- An outgoing row waits at most 30 seconds for its Timeline echo. Transport errors can end that wait earlier. Failure or unconfirmed delivery stops the pulse and removes only the local row after 10 seconds. Definitive rejection uses red alert feedback; a transport, timeout or unknown-operation outcome stays neutral and says that the send or conversation is unconfirmed. A late echo replaces an unconfirmed row while retained, and always renders normally after removal. Late acknowledgements cannot restart the timeout; the client never automatically resends a message (`packages/agent-remote-web/src/replica/store.ts`, `packages/agent-remote-web/src/client/remote-session-client.ts`, `packages/agent-remote-web/src/react/OutgoingMessageItem.tsx`).
