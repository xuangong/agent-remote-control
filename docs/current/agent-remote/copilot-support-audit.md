...
- Enabled user-invocable skills come from native discovery. `commands.invoke` resolves body/arguments and native display text. Documentation revalidates the current locator and reads a regular file without following a final symlink, at most 256 KiB. It is a current document read, not an immutable output attachment. Tool previews use the common 65,536-character/128-block bound.
