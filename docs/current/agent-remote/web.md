...
The Timeline renders direct native children below the final assistant entry in their originating parent turn. Each child appears once, ordered by creation or first-discovery time; status changes do not reorder it. Missing or not-yet-rendered origin turns use a separately labeled Session subagents disclosure, collapsed by default. Its heading shows the child count; expansion survives status updates and resets when switching sessions. Children associated with a visible reply remain expanded. Child rows expose title, supporting role/task information and status, and invoke a caller-owned navigation callback. The Web renderer does not parse native Codex events or create sessions itself. Tool details can carry a normalized `sessionReference` with a native session ID and display title. The host resolves it to a same-family navigation link; the tool header links the title separately from its details toggle. Missing or out-of-family references remain text, and navigation failures remain visible.
