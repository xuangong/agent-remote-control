...
function validBroker(value: unknown): value is RemoteHostBrokerState {
  ...
  if (!value.keys.every((item: unknown) => Array.isArray(item) && item.length === 2 && /^[a-f0-9]{64}$/.test(item[0]) && record(item[1]) && time(item[1].expires) && (item[1].installationId === undefined || string(item[1].installationId))) ||
