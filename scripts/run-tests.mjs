...
const commands = {
  ...
  'controller-updates': ['--filter', '@orchardworks/agent-remote-controller', 'exec', 'vitest', 'run', 'src/controller-update.test.ts', 'src/update-command.test.ts', 'src/controller-launcher.test.ts', 'src/controller-shutdown.test.ts', 'src/host.test.ts', ...(process.platform === 'win32' ? [] : ['src/cli-linux.test.ts', 'src/systemd.test.ts']), '--testTimeout=15000', '--hookTimeout=15000', '--maxWorkers=1'],
