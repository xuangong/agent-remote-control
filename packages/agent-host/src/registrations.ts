...
type HostProviderId = 'codex' | 'claude';
...
export function selectedHostProviders(env: NodeJS.ProcessEnv): HostProviderId[] {
  ...
  for (const provider of selected) {
    if (provider !== 'codex' && provider !== 'claude') throw new Error(`Unknown or empty Agent Host provider: ${provider || '(empty)'}. Use codex, claude, or codex,claude.`);
