...
Every client mutation has a stable operationId. SessionWire mutations and owner create requests carry it unchanged through Relay to Host. Shared-recipient create crosses the existing quota namespace boundary: Relay keys the reservation by stable Host ID, subject, Provider and client operation ID, then deterministically derives the UUID sent to Host. That Host UUID remains stable across retries, reconnects, broker restart, credential rotation and revoke/regrant for the same identities. Each new user intent creates a new client ID, including identical text; retries reuse the same ID. Keep requestId for response correlation and native approval request IDs for native interaction identity. Within the retained cache window, the same operationId with different intent is a conflict.
...
The user explicitly requires automatic cleanup so logs cannot grow indefinitely. Diagnostic logs and in-memory operation-cache entries have different retention rules. Rotate diagnostic output by size and retain a fixed number of archives; handle inherited launchd/manual daemon output descriptors correctly. Default target is 5 MiB per diagnostic file and three archives. The active threshold is soft: one bounded owned line may cross it before the next owned-write check or cleanup interval. The daemon must enforce cleanup while running as well as at startup, without requiring a restart. Rotation failure must not create an unbounded memory queue or log recursion.
