...
The native `codex-daemon-client` routes notifications and original server requests by native thread ID over the parent's app-server connection, owns discovery buffers and reconciles authoritative recovery snapshots. The Provider runtime maps its callbacks to SDK child descriptors and session observations. Discovery preserves direct parentage and stable spawn provenance, updates parent `childSessions` summaries, and receives child activity before a child chat is opened. Opening a child reuses this runtime rather than starting another app-server or cold-resuming an unavailable native child. The child's normal session handles its Timeline, resource reads, commands and pending interaction responses.

Discovery reads both `collabAgentToolCall` and `subAgentActivity` from live notifications and saved parent history. Native thread metadata must establish parentage before the runtime requests full child history. Child titles prefer the native agent path, including `source.subAgent.thread_spawn.agent_path`, then fall back to the native name, nickname, role, or thread ID. Parent-controlled children remain visible and their history can be opened even when direct input is disabled.
...
When Claude child history reveals an input before an already-streamed answer, the adapter submits the complete ordered Timeline through the internal SDK replacement item. Relay reuses its existing epoch replacement notification so open views and reconnects recover the input in order. Forwarded messages absent from an SDK catalog read retain their position relative to known native message identities; equal text with different native UUIDs remains distinct. This includes repeated parent `SendMessage` work on the same native child.

## Copilot official SDK and parent-owned tasks

Copilot runs through `@github/copilot-sdk` 1.0.11 and CLI 1.0.83 in Agent Host. The adapter owns `CopilotClient`/SDK stdio, native callback interpretation, interaction-aware turn projection and resources. Host supplies provider/workspace discovery and native identity loading; Relay and renderer use unchanged public contracts. Copilot is explicitly selected in the launcher or `AGENT_HOST_PROVIDERS`.

Native `send` modes supply immediate/queued delivery. Native interaction IDs group tool steps and steering; queued interactions become distinct public turns even when native idle arrives only once. Saved history uses the same distinction. Child sessions are task/event-log views over the loaded parent runtime, with native repeated input and cancel; questions/approvals remain on the parent. Model/skills/tasks/history RPCs are public experimental interfaces with conditional availability. See the [Copilot audit](copilot-support-audit.md) for exact mappings, limits, resource bounds and native evidence; this implementation does not use ACP.

Codex `subAgentActivity` records expose a normalized optional tool-detail `sessionReference` containing `agentThreadId` as the native session ID and `agentPath` as the display title, falling back to the ID. Live and historical projection share this mapping. Navigation authorization remains with the Host directory's existing native-family attachment checks.

Codex `collabAgentToolCall` waits normalize all unique `receiverThreadIds` into tool-detail `sessionReferences`, including while the call is running. An empty receiver list denotes a wait for updates from any sub-agent, as used by native `wait_agent`; it does not denote waiting for all children to finish. Missing target metadata is labeled unavailable. Completion describes the wait as finished without claiming any particular child completed. Live and historical projection share this interpretation.
