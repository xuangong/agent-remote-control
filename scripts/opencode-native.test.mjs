...
import { mkdtemp, mkdir, writeFile, copyFile, realpath, rm } from 'node:fs/promises';
...
test('native OpenCode serves shared normalized sessions with isolated local model transport', { skip: !executable, timeout: 120000 }, async () => {
  ...
  try {
    ...
    assert.equal((await fetch(`${serverUrl}/global/health`, { headers })).status, 200);
    console.log(JSON.stringify({ ardbSharedClients: 2, native: 'OpenCode', model: 'isolated HTTP fixture', requests, observations: events.length, nativeSessionId: runtime.sessionId, rename: true, resume: true, nativeHistoryPagination: true, crossProjectListing: true, sharedServerSurvivedDispose: true }));
  } finally {
