...
import type { AgentSnapshot, ClientMessage, HistoryPage, ServerMessage, TimelineDirection } from '@borgee/agent-remote-protocol';
import { HttpWebSocketTransport, RemoteOperationError, type RemoteAgentTransport, type RemoteConnection, type RemoteProtocolObservation, type RemoteTransportDiagnostic, type RemoteTransportListener } from '@borgee/agent-remote-web/headless';
...
class FakeTransport implements RemoteAgentTransport {
  ...
  connect(_agentId: string, listener: RemoteTransportListener): RemoteConnection {
    ...
    if (this.fast) {
      queueMicrotask(() => {
        this.open();
        this.emit({ protocolVersion: '1.1.0', type: 'negotiated' });
