...
describe('Remote Host broker', () => {
  ...
  it('forwards catalog reads and attaches with a stable browser binding', async () => {
    ...
    expect((await fetch(f.url + `/v1/sessions/${attached.agentId}/snapshot?protocolVersion=1.1.0`)).status).toBe(200);
  });
...
it('carries native-host discovery, creation, snapshots, and chat through the production uplink client', async () => {
  ...
  const connect = () => createRemoteHostUplinkClient({
    ...
    async control(request) {
      ...
      if (!existing) await relay.createAgent({ protocolVersion: '1.1.0', type: 'create_agent', payload: { requestId: request.sessionId!, agentId: request.sessionId!, providerId: 'recorded', config: { sessionId: body.nativeSessionId } } });
