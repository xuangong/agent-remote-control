...
describe('transport-neutral HTTP execution', () => {
  it('returns the same encoded public result through the executor and Node HTTP adapter', async () => {
    ...
    for (const path of ['/v1/providers?protocolVersion=1.1.0', '/v1/providers',
      '/v1/sessions/missing/snapshot?protocolVersion=1.1.0', '/v1/sessions/%ZZ/timeline?protocolVersion=1.1.0']) {
