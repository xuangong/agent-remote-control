...
const fields = { host: 'hostId', agent: 'agentId', provider: 'providerId', session: 'nativeSessionId', parent: 'parentNativeSessionId' } as const;
...
export function readControllerLocation(query: URLSearchParams): ControllerLocation {
  ...
  for (const [key, field] of Object.entries(fields)) {
    ...
    if (values.length !== 1 || !value || value.length > 4096 || /[\u0000-\u001f\u007f]/.test(value) ||
      ((key === 'host' || key === 'provider') && !/^[A-Za-z0-9_-]{1,256}$/.test(value))) throw new Error('Invalid session link.');
    result[field] = value;
  }
