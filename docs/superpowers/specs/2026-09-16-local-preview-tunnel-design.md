...
A registration records its ID, principal/Host binding, canonical target, creation time, expiry policy and computed `expiresAt`, source references, lifecycle state, and revision. The frontend may display a countdown but cannot expire a registration or extend a lease. The Controller evaluates deadlines independently of page visits and publishes changes. Exact TTL defaults and whether a policy is fixed-duration or idle-based have not been selected in the conversation; implementation must expose the chosen policy explicitly instead of inferring one from the UI. Use `expiresAt: null` only for an explicitly configured no-deadline policy.
