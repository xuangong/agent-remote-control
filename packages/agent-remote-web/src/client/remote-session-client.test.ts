...
function live(
  ...
  return {
    protocolVersion: '1.1.0', type: 'agent_stream',
    payload: {
...
describe('RemoteSessionClient', () => {
  it('correlates planning acknowledgement without optimistically changing authoritative state', async () => {
    ...
    expect(transport.sent.at(-1)).toEqual({ protocolVersion: '1.1.0', type: 'set_planning', payload: { requestId: 'planning-one', agentId: 'agent-one', active: true } });
    const acknowledgement = { protocolVersion: '1.1.0', type: 'command_acknowledged', payload: { requestId: 'planning-one', agentId: 'agent-one', command: 'set_planning' } } as const;
  ...
  it('uses subscription acknowledgement as the barrier before tail catch-up and live convergence', async () => {
    ...
    transport.emit({ protocolVersion: '1.1.0', type: 'negotiated' });
    ...
    transport.emit({
      protocolVersion: '1.1.0', type: 'timeline_subscribed',
  ...
  it('supersedes a multi-page after fetch when the relay replaces the Timeline epoch', async () => {
    ...
    transport.emit({
      protocolVersion: '1.1.0', type: 'timeline_replacement',
  ...
  it('applies pushed resource replacement and terminal metadata without polling', async () => {
    ...
    transport.emit({
      protocolVersion: '1.1.0', type: 'timeline_resource_binding_replaced',
      payload: {
    ...
    transport.emit({
      protocolVersion: '1.1.0', type: 'resource_update',
      payload: {
  ...
  it('keeps operations with the same request ID isolated by their expected response', async () => {
    ...
    transport.emit({
      protocolVersion: '1.1.0', type: 'protocol_error',
