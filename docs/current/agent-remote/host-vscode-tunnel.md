...
This implementation supports macOS and Linux process groups and Windows Job Objects. Windows resolves the installation's `code-tunnel.exe`, creates the native process suspended, assigns it to a kill-on-close job, and resumes it. The job owner watches the supervisor process handle as well as the native process handle, so a supervisor crash or early native exit also reclaims descendants. Windows PowerShell hosts the job without a visible window. It does not install a VS Code background service or adopt an existing singleton; an attached-instance report stops the managed client and reports failure. Existing manually managed tunnels are untouched.
