...
describe('session wire negotiation', () => {
  it('acknowledges the exact planning command after provider completion', async () => {
    ...
    output.length = 0;
    const submitted = wire.receive(JSON.stringify({ protocolVersion: '1.1.0', type: 'set_planning', payload: { requestId: 'planning-1', agentId: 'agent-1', active: true } }));
    await Promise.resolve();
    expect(selected).toEqual([true]);
    expect(output).toEqual([]);
    finish();
    await submitted;
    expect(output).toEqual([{ protocolVersion: '1.1.0', type: 'command_acknowledged', payload: { requestId: 'planning-1', agentId: 'agent-1', command: 'set_planning' } }]);
    wire.close();
  });
...
describe('session wire Timeline and manager-event projection', () => {
  it('fails closed when a manager event cannot be encoded', async () => {
    ...
    emit({ type: 'agent_state', agentId: 'agent-1', snapshot: agent.snapshot() });
    await wire.receive(JSON.stringify({
      protocolVersion: '1.1.0', type: 'send_message',
  ...
  it('acknowledges an accepted session command with its request correlation', async () => {
    ...
    expect(output).toEqual([{
      protocolVersion: '1.1.0', type: 'command_acknowledged',
