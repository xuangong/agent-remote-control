...
`/ask` toggles Ask on or off for this browser, with the preference saved locally and disabled by default. Enabling shows the floating Ask button; clicking it opens a small non-modal conversation above the focused source. Disabling hides both surfaces and releases Ask content and activity subscriptions, while retaining the conversation and drafts. The Ask composer also accepts `/ask` to disable the feature. `/ask question` enables Ask, opens the window, and sends the question through that conversation's normal client and persistent outbox; it does not replace an existing Ask draft. Ask uses the same source-bound read tool and developer instructions as Side, without copying source history, adding a source badge, or sending a provenance prefix as user text. It inherits the same Host/provider support requirements.

The Ask window starts in Content only mode. Mobile windows stay centered in the usable visual viewport; on desktop widths above 1180px, the window opens beside the floating Ask button, flips above when needed, and stays within the screen edges. When a mobile keyboard obscures the layout or the available height is small, it fills the remaining visual viewport with an 8px vertical inset. The heading and composer stay visible while the timeline and multiline input scroll independently; focus reveal cannot scroll the floating frame or the background document. Viewport sizing is resampled briefly after focus, orientation, and page-resume events to catch delayed standalone keyboard measurements without continuous polling. A small toggle next to Clean switches between Content only and Simple without changing the main conversation's display preference. The floating Ask button uses the same drag and keyboard positioning behavior as Track, with its own saved position. On desktop, dragging the window heading moves the window and its minimized button together; the position survives minimizing and reload. Window dragging never moves the centered mobile layout.

Each source has its own Ask in a tab-local ledger. Minimizing releases its view of the content subscription, retains the draft and conversation, and leaves the native runtime running. The content subscription stops unless Track or another mounted view still retains it. The focused source's active Ask remains on the shared activity channel, outside the Track menu and counts. Its button uses a green background for working, yellow for pending, and the white surface for idle. Unread pending and working-to-idle transitions use Track's yellow and gray pulses; opening Ask acknowledges them. Clean replaces only the auxiliary Ask observer, and restoring the page reattaches its saved native identity for activity observation. Questions waiting for the window to reconnect remain saved in session storage; uncertain sends remain visible with explicit Retry/Delete through normal message recovery. Clean creates a fresh referenced session, switching only after creation and settings inheritance succeed. It clears the old draft unless edited during the request; it does not delete the old native session. Failed or unknown creation outcomes retain the previous conversation and reuse the same creation operation on retry. The compact composer keeps normal message, image, and interaction behavior; visual-viewport sizing keeps the floating window inside a mobile keyboard viewport.
...
On compact screens, Sessions & Hosts keeps the account row, section navigation, and New session action outside a single scrolling body. Sessions shows the Host selector, collapsible environment filter/details, and session discovery first; previews follow the list. Controller updates and VS Code tunnel management live only under Settings. Controller update state stays mounted when switching panels. Native Host selectors contain long option text so WebKit cannot widen the sidebar scrollport. Favorites and creation remain separate panels. Track is visually hidden while a supporting dialog is open without stopping its observations. Disclosure controls participate in the drawer focus loop; closed disclosure contents do not.

The desktop sidebar separates Sessions, Favorites, New session, and Settings. Sessions is the default panel, with Host and Provider selection, search over loaded sessions, and two-line session titles. Creation settings and Host management occupy their own panels while form drafts remain mounted. The heading and panel navigation stay visible above one scrolling content area; changing panels resets that area to its beginning.
...
Favorites is an account-wide bookmark tree alongside Sessions, independent of the selected Host or Provider. The root contains folders and sessions in manual mixed order. Existing favorites appear at root in their previous order. Folders can nest up to eight levels and support inline creation in the folder picker, rename, move, and confirmed subtree deletion. An account can keep 128 folders and 256 session bookmarks. A bookmark remains unique by Host, Provider and native session ID. Editing a prompt into a new fork preserves its bookmark ID, folder and position.

The sidebar and mobile title shortcut share compact tree rows with secondary Host/Provider metadata and overflow actions. Folder expansion persists per account in this browser. Clicking a star opens Add/Edit favorite with a folder picker (root by default for new favorites), inline New folder, Save and Remove favorite. Track/Untrack lives in the row menu, with a small tracked marker on the row. Tracking remains client-scoped and independent of folder edits. The sidebar offers selectable favorites-count and tracked-count tags. The tracked tag filters all tracked bookmarks, including sessions in collapsed folders, into a flat list with the same row menus. The floating tracking panel opens sessions and acknowledges changes; it has no per-session Untrack button. Removing a bookmark or deleting its folder subtree also removes local tracking after a confirmed favorites snapshot and migration recovery. Failed reads or writes never clear tracking; offline and inaccessible bookmarks remain tracked. Fork migrations are recovered before cleanup so a replacement bookmark does not look like a deletion. Cleanup releases retained background connections through the existing tracking lifecycle and does not stop native sessions.

Track also retains content subscriptions that this page has already opened. Adding a tracked session does not preconnect or load its conversation history. After its first open, switching away detaches the view while keeping the content client, replica, and message recovery active; reopening reuses the current ready or recovering client without restarting synchronization. Main, side, and Ask views share the same connection collection. A subscription is released once it is neither tracked nor held by a mounted conversation view. Background tracked content is released when workspace access becomes unavailable; account/transport changes and page teardown clear the collection. These retained subscriptions remain subject to normal transport recovery and server authorization.


Drag a row handle between siblings to reorder or onto a folder to move inside. Hovering opens a collapsed folder; dragging near the list edges scrolls it. On touch screens, hold the handle before moving. The rest of the row remains available for normal scrolling. Move to, Move up and Move down also work without dragging. Tree arrows navigate and expand, Enter opens, F2 renames a folder, and Shift+F10 opens its actions. Long names truncate visually while full names remain available to assistive technology and in hover details.

Hosted GET/POST `v1/favorites` reads and mutates a durable account tree using a revision. The Relay serializes changes, rejects cycles or stale edits, and commits folder and session changes atomically. A conflict refreshes the tree without replaying the operation; the user reviews and retries. Legacy `v1/stars` operations preserve folder placement and advance the same revision. Cloudflare persists folders separately from session bookmarks and restores both on restart. Native session and Controller protocols do not change.
...
The share dialog keeps the QR code and Copy link visible, with the full URL and local resume command collapsed. Scan beside Discover sessions opens a camera scanner even before a session is selected. On mobile, the title's Favorites popover also offers Scan to open, using the same scanner without opening the sidebar. Scanning selects a session in the current workbench, preserving local drafts and navigation history. Favorites remain account-scoped; tracking remains client-scoped. Neither is changed automatically by scanning.

The scanner prefers the rear camera and loads its decoder only on demand. It checks bounded video frames at most five times per second and releases camera tracks after a result, on close, and when the page is hidden. Returning from the background requires Resume camera. Permission and camera failures offer retry and a pasted-link fallback. Only session URLs from the current Controller origin are accepted; the existing native identity attachment flow checks access and ignores stale runtime Agent IDs.
...
Acceptance tests decode generated QR video frames, verify clipboard contents and in-page switching with retained drafts, exercise camera cleanup and denied permissions, follow desktop-to-phone-to-desktop authentication with empty browser storage, and reject an unauthorized account over real HTTP/WebSocket transports with a deterministic Provider. Physical camera scanning and native CLI session retention remain separate acceptance checks.
