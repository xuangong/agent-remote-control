...
- An outgoing row waits at most 30 seconds for its Timeline echo. Transport errors can end that wait earlier. Failure or unconfirmed delivery stops the pulse, displays red feedback, and removes only the local row after 10 seconds. A late echo replaces an unconfirmed row while retained, and always renders normally after removal. Late acknowledgements cannot restart the timeout; the client never automatically resends a message (`packages/agent-remote-web/src/replica/store.ts`, `packages/agent-remote-web/src/client/remote-session-client.ts`).
