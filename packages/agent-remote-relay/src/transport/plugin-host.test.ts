...
const negotiate = JSON.stringify({ protocolVersion: '1.1.0', type: 'negotiate' });
...
describe('socket-free plugin uplink host', () => {
  ...
  it('serves the production provider, snapshot, and Timeline reads through an attached target', async () => {
    ...
    expect(requests).toEqual([
      '/remote/attach', 'negotiate', '/v1/providers?protocolVersion=1.1.0',
      '/v1/sessions/remote-one/snapshot?protocolVersion=1.1.0',
      '/v1/sessions/remote-one/timeline?protocolVersion=1.1.0&requestId=remote-http-1&direction=tail&limit=10',
    ]);
  ...
  it('rejects foreign Agent HTTP routes and native-session import', async () => {
    const f = await fixture();
    f.receive({ type: 'rpc_request', requestId: 'foreign', method: 'GET', path: '/v1/sessions/other/snapshot?protocolVersion=1.1.0' });
  ...
  it('serializes commands within a stream while another browser remains responsive', async () => {
    ...
    f.receive({ type: 'stream_message', streamId: 'one', message: JSON.stringify({ protocolVersion: '1.1.0', type: 'send_message',
      payload: { requestId: 'hold', agentId: 'agent-one', text: 'hold' } }) });
    f.receive({ type: 'stream_message', streamId: 'one', message: JSON.stringify({ protocolVersion: '1.1.0', type: 'timeline_request',
  ...
  it('retires cancelled RPC delivery without replaying or undoing its Provider action', async () => {
    const f = await fixture();
    f.receive({ type: 'rpc_request', requestId: 'cancelled', method: 'GET', path: '/v1/providers?protocolVersion=1.1.0' });
    f.receive({ type: 'rpc_cancel', requestId: 'cancelled' });
    await new Promise((resolve) => setImmediate(resolve));
    expect(f.sent).toEqual([]);
    f.receive({ type: 'rpc_request', requestId: 'next', method: 'GET', path: '/v1/providers?protocolVersion=1.1.0' });
