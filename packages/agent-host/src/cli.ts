...
async function main(): Promise<void> {
  ...
  else if (command === 'claude') process.exitCode = await runClaudeCommand(args.slice(1), stateDir, process.env);
  else if (command === 'opencode') process.exitCode = await runOpenCodeCommand(args.slice(1), stateDir, process.env);
  else if (command === 'copilot') process.exitCode = await runCopilotCommand(args.slice(1), stateDir, process.env);
...
async function serveConfigured(daemon: boolean, diagnosticLog: DiagnosticLog | undefined, diagnosticSecrets: Set<string>): Promise<void> {
  ...
  let daemonMonitor: ReturnType<typeof createDaemonMonitor> | undefined;
  const providerDiscovery = await createHostProviderDiscovery({ stateDir, env: environment,
    onDiagnostic: (line) => {
      writeDiagnostic(line, diagnosticSecrets);
      if (line.includes('"outcome":"unavailable"') || line.includes('"phase":"initialize"')) void daemonMonitor?.sample('native_connection');
    } });
  if (supportsCodexDaemonControl(environment)) {
    try {
      daemonMonitor = createDaemonMonitor(stateDir, environment.AGENT_REMOTE_CODEX_HOME ?? environment.CODEX_HOME ?? join(homedir(), '.codex'));
      void daemonMonitor.sample('controller_start');
    } catch { /* Diagnostic storage must not prevent Host startup. */ }
  }
  ...
  const discoveryTimer = setInterval(() => { void providerDiscovery.refresh().catch(() => undefined); }, 60_000);
  ...
  const shutdown = async () => {
    ...
    await providerDiscovery.stop();
    daemonMonitor?.dispose();
    management.close();
