...
Operation settlement is owned by the Relay service and shared by standalone WebSocket/session-channel, plugin uplink, Controller Host, and DSH entry points. The Host supplies the same cache used for native creation and rename. Closing a browser or reconnecting an uplink does not replace this service or cancel admitted native work. Service shutdown drains the cache once; replacing the service starts a new guarantee scope.

Trusted integration state supplies the authority scope and native target. Public binding IDs and transport request IDs are not deduplication identities. An operation ID reuses its retained outcome only for the same scope, kind, native target and canonical parameters; changed intent returns `operation_conflict`. Ephemeral sessions without a native identity receive a service-owned incarnation identity, distinct after replacement. Current authorization and control are checked before cache lookup, including completed receipts. A writable session wire without a runtime settlement executor rejects mutations.

The bounded in-memory cache never evicts in-flight work; terminal outcomes expire after the retention interval. Capacity exhaustion rejects before dispatch. Validation failures are retained rejections. Only `AgentOperationRejectedError` explicitly supplied by a native adapter proves that a dispatch was rejected without side effects. Generic post-dispatch failures remain `operation_outcome_unknown`; `native_file_limit` retains its diagnostic code with the same uncertain semantics. Lifecycle hooks may reject admission, retain resources and mark uncertainty, but do not override settlement or replay native work.
...
### Identified standalone creation and resume
