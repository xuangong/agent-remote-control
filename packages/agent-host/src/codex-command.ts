...
export async function runCodexCommand(args: string[], stateDir: string, environment: NodeJS.ProcessEnv): Promise<number> {
  ...
  const executable = resolveNativeExecutable(configured.AGENT_HOST_CODEX ?? configured.AGENT_REMOTE_CODEX_EXECUTABLE ?? 'codex', '@openai/codex/bin/codex.js', configured);
  ...
  const env: NodeJS.ProcessEnv = { ...sanitizeNativeEnvironment(configured), CODEX_HOME: home, LC_ALL: 'C' };
  ...
  if (daemon) {
    ...
    if (shared && process.platform === 'win32') {
      ...
      env.CODEX_REMOTE_AUTH_TOKEN = endpoint.token;
      nativeArgs = ['--remote', endpoint.url, '--remote-auth-token-env', 'CODEX_REMOTE_AUTH_TOKEN', ...sessionArgs];
    } else nativeArgs = shared ? ['--remote', `unix://${socket}`, ...sessionArgs] : args;
  }
