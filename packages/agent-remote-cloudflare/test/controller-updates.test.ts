...
it('persists Host versions and restricts upgrade RPC to the owner over real Worker HTTP and WebSocket', async () => {
  const release = { protocolVersion: '1.5.0', version: '0.2.0', revision: 'a'.repeat(40), sha256: 'b'.repeat(64), asset: 'orchardworks-agent-remote-controller-0.2.0.tgz', nodeMajor: 22, platforms: ['linux-x64'] };
  ...
  const identity = { version: '0.1.0', revision: 'c'.repeat(40), platform: 'linux', arch: 'x64', nodeMajor: 22, remoteUpdate: true };
