...
The browser uses a right-side dialog on desktop with an expand control, and fills the viewport on mobile with safe-area padding. The underlying conversation stays mounted, preserving its scroll position and draft. Closing the dialog removes the iframe and aborts pending entry requests without unregistering the port. Expired or unregistered previews and Controller disconnections remove the iframe and display the current access state. Entry requests and initial page loading have a 20-second deadline or loading notice.
