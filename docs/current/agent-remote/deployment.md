...
```sh
pnpm --filter @borgee/agent-remote-cloudflare exec wrangler secret put AGENT_REMOTE_SIGNING_SECRET
pnpm --filter @borgee/agent-remote-cloudflare exec wrangler deploy
```
