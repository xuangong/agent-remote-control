...
async function fixture() {
  ...
  const native = () => ({ id: 'ses_test', title, directory: process.cwd(), time: { created: 1000, updated: 2000 } });
  ...
  return { url: `http://127.0.0.1:${(server.address() as any).port}`, requests, emit, native, set globalSessions(value: any[]) { globalSessions = value; }, set globalListingStatus(value: number) { globalListingStatus = value; }, set connectedProviders(value: string[]) { connectedProviders = value; }, get connections() { return connections; }, get activeConnections() { return streams.size; }, set failHistoryReads(value: number) { failHistoryReads = value; }, blockNextHistory() {
  ...
  }, get messages() { return messages; }, set messages(value) { messages = value; }, set status(value) { status = value; }, set permissions(value) { permissions = value; }, set questions(value) { questions = value; }, set rejectPrompt(value) { rejectPrompt = value; }, disconnect() { for (const stream of streams) stream.destroy(); } };
}
...
test('tracks external native model and agent changes after a submitted choice but retains unsent selections', async () => {
  const f = await fixture(); const provider = new OpenCodeAgentProvider({ serverUrl: f.url }); cleanups.push(() => provider.close());
