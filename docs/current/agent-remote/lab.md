...
The Ask window is centered in the visual viewport and starts in Content only mode. A small toggle next to Clean switches between Content only and Simple without changing the main conversation's display preference. The floating Ask button uses the same drag and keyboard positioning behavior as Track, with its own saved position.
