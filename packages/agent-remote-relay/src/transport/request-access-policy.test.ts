...
const routes = [
  ['GET', '/v1/providers?protocolVersion=1.5.0'],
  ['GET', '/v1/sessions/private/snapshot?protocolVersion=1.5.0'],
  ['GET', '/v1/sessions/private/timeline?protocolVersion=1.5.0'],
  ['POST', '/v1/sessions'],
...
describe('Agent Remote request access policy', () => {
  ...
  it('allows authenticated reads and preserves typed missing-session errors', async () => {
    ...
    const response = await fetch(`${url}/v1/providers?protocolVersion=1.5.0`, { headers });
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ type: 'provider_list', payload: { providers: [] } });
    const missing = await fetch(`${url}/v1/sessions/private/snapshot?protocolVersion=1.5.0`, { headers });
