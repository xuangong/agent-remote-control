...
export async function createClaudeHostRegistration(options: ClaudeHostRegistrationOptions = {}): Promise<AgentHostProviderRegistration> {
  ...
  const env = { ...process.env, ...options.env, ...(options.claudeHome ? { CLAUDE_CONFIG_DIR: options.claudeHome } : {}) };
  ...
  const provider = new ClaudeAgentProvider({ executable, env, requestTimeoutMs: options.requestTimeoutMs, onDiagnostic: options.onDiagnostic });
